...
class StopCurrentAttemptRequested(TaskInterruption):
    """指定账号被手动停止。"""

    def __init__(self, message: str = "当前账号已手动停止"):
        super().__init__(message)
...
class RegisterTaskControl:
    ...
    def __init__(self):
        ...
        self._attempt_id_to_index: dict[int, int] = {}
        self._attempt_index_to_id: dict[int, int] = {}
        self._skip_active_attempt_ids: set[int] = set()
        self._stop_active_attempt_ids: set[int] = set()
    ...
    def request_stop_attempt(self, attempt_index: int) -> bool:
        with self._lock:
            attempt_id = self._attempt_index_to_id.get(int(attempt_index))
            if attempt_id is None:
                return False
            self._stop_active_attempt_ids.add(attempt_id)
            return True

    def start_attempt(self, attempt_index: int | None = None) -> int:
        with self._lock:
            ...
            if attempt_index is not None:
                ...
                self._attempt_id_to_index[attempt_id] = idx
                self._attempt_index_to_id[idx] = attempt_id
            return attempt_id
    ...
    def finish_attempt(self, attempt_id: int | None) -> None:
        ...
        with self._lock:
            ...
            self._stop_active_attempt_ids.discard(attempt_id)
            attempt_index = self._attempt_id_to_index.pop(attempt_id, None)
            if attempt_index is not None:
                self._attempt_index_to_id.pop(attempt_index, None)
    ...
    def checkpoint(
        ...
        *,
        ...
        with self._lock:
            ...
            if attempt_id is not None and attempt_id in self._stop_active_attempt_ids:
                self._stop_active_attempt_ids.discard(attempt_id)
                raise StopCurrentAttemptRequested()
            if consume_skip:
                if (
    ...
    def snapshot(self) -> dict[str, Any]:
        with self._lock:
            return {
                ...
                "targeted_stop_attempts": len(self._stop_active_attempt_ids),
            }
...
class RegisterTaskStore:
    ...
    def request_stop_attempt(self, task_id: str, attempt_index: int) -> dict[str, Any] | None:
        control = self.control_for(task_id)
        if not control.request_stop_attempt(attempt_index):
            return None
        return control.snapshot()
